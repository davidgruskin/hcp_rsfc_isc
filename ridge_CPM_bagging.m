...
%%
function   [mdl_bagged, ridgeCPMoutput ] = ridge_CPM_bagging(hcp_fc,hcp_isc,all_used, sub_data_cat,covariates,family_ids, num_iter, randomize_bin,perm_sub_order)
%
...
%
%               covariates             Matrix containing mean FD and tSNR values 
%                                      [subjects x scans]
%
...
covariates        = covariates(all_used,:);
...
mse_output    = zeros(num_loops,1+size(covariates,2)/2);
qs_output     = zeros(num_loops,1);
rrank_output  = zeros(num_loops,1);
...
for loop = 1:num_loops
    ...
    parfor iter = 1:num_iter
        ...
        % structure)
        randomsplit      = zeros(num_sub_total,2);
        randomsplit(:,1) = 1:num_sub_total;
        ...
        hcp_isc1 = NaN(num_sub_total,num_node);
        for subj = 1:num_sub_total
        ...
        % for later use
        all_behav1 = cat(2,nanmean(conv_r2z(hcp_isc1(:,:,1)),2),covariates(:,[1:2:end]));
        if randomize_bin == 1
            all_behav_tmp = all_behav1(perm_sub_order(:,iter),:);
            % Make sure the mean FD/tsnr values from the rest scans are kept unshuffled
            all_behav_tmp(:,[2,4]) = covariates(:,[1,5]);
        ...
        for i_fold = 1 : k
            ...
            beta      = beta(1:size(beta,1)-(size(covariates,2)/2),:);
    ...
    all_mats2  = hcp_fc{2,1}(:,:,all_used);
    all_behav2 = cat(2,nanmean(conv_r2z(hcp_isc(all_used,:,2)),2),covariates(:,[2:2:end]));
...
% Store outputs
ridgeCPMoutput.rrank         = rrank_output;
